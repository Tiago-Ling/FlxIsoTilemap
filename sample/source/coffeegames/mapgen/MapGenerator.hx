...
class MapGenerator {
	...
	private var cNW_CORNER:Int;
	private var cNE_CORNER:Int;
	private var cSE_CORNER:Int;
	private var cSW_CORNER:Int;
	private var cN_WALL:Int;
	private var cE_WALL:Int;
	private var cS_WALL:Int;
	private var cW_WALL:Int;
	private var cS_DOOR:Int;
	private var cE_DOOR:Int;
	private var cENTRANCE:Int;
	private var cFLOOR:Int;
	private var cNOTHING:Int;
	...
	public function init():Void {
		...
		map = new BitmapData(width, height, false, cNOTHING);
		if (showDebug) {
...
/*		trace("NW CORNER (" + iNW_CORNER + ") : " + cNW_CORNER);
	...
	private function printRoom(room:Room, uDoors:Array<Door>):Void {
		...
		for (countY in startY...endY) {
			...
			for (countX in startX...endX) {
				...
				if (countX == startX && countY == startY) { // NW CORNER
					...
					for (i in 0...uDoors.length) {
						if (countX == uDoors[i].x && countY == uDoors[i].y) {
							map.setPixel(countX, countY, cS_WALL);
							room.layout[indexCountY][indexCountX] = iS_WALL;
							mapData[countY][countX] = iS_WALL;
							continue;
						}
					}
					
					map.setPixel(countX, countY, cS_DOOR);
					room.layout[indexCountY][indexCountX] = iS_DOOR;
					mapData[countY][countX] = iS_DOOR;
					...
					map.setPixel(countX, countY, cFLOOR);
					room.layout[indexCountY][indexCountX] = iFLOOR;
					mapData[countY][countX] = iFLOOR;
				}
